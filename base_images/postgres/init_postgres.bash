...
POSTGRES_CONFIG_FILE=/etc/postgresql/9.5/main/postgresql.conf
...
if [[ "$(cat ${POSTGRES_CLUSTER_DIR}/PG_VERSION)" == '9.3' ]]; then
    ...
    mv /etc/postgresql/9.5 /etc/postgresql/9.3
    sed -i 's/9\.5/9\.3/g' /etc/postgresql/9.3/main/postgresql.conf
...
[ $(cd $POSTGRES_CLUSTER_DIR && ls -lA | wc -l) -ne 1 ] || \
    su postgres -c "usr/lib/postgresql/9.5/bin/initdb -D ${POSTGRES_CLUSTER_DIR} -E utf-8 --locale=en_US.UTF-8"
...
pg_ctlcluster 9.5 main start -o '-c listen_addresses=""' # Temporarily start Postgres for local connections only.
